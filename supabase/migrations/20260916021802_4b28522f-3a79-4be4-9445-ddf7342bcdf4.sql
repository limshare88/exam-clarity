-- The 3 flat hand-drawn SVG backgrounds (Cozy Study Room, Starry Night Bedroom, Bright
-- Classroom) are retired now that illustrated image-based backgrounds exist -- they
-- looked noticeably worse by comparison once there was something to compare them to.
-- Clean up any existing inventory rows for them (equipped or not), same as the
-- hairstyle removal earlier, so nothing dangling is left for items no longer purchasable
-- or renderable (BackgroundScenes.tsx no longer has the SVG components at all).
DELETE FROM public.gamification_inventory
WHERE item_id IN ('bg-study', 'bg-night', 'bg-classroom');
